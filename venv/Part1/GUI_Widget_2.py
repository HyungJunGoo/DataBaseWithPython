...
class MainWindow(QMainWindow):      # QWidget > QMainwindow 클래스의 서브 클래스
    def __init__(self):
        super().__init__()
        self.setupUI()
    ...
    def setupUI(self):
        ...
        # 스테이터스 바 설정 (QMainWindow 객체)
        self.statusBar = QStatusBar(self)
        self.setStatusBar(self.statusBar)

        # 레이블과 라인에디트 설정
        self.label = QLabel("이름: ", self)
        self.label.setGeometry(50,20, 50, 20)

        self.lineEdit = QLineEdit("", self)
        self.lineEdit.setGeometry(100, 20, 200, 20)
        ...
        self.btnQuit.resize(self.btnClear.sizeHint())
